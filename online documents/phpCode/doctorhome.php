...
function get_file_extension($file_name) {
 ...
 require 'functions/psg.php';
  $codx=Generator();

 //$cod=$_SESSION['con_id'];
require 'headwithsearch.php';

//require 'menu_test.php';
...
mysql_query("INSERT INTO `mydocs` 
	(`docs_id`, `img`, `current_folder`, `img_name`, `details`, `doc_date`, `doctor_name`, `client_name`, `client_phone`,`cod`,`cat`)
	 VALUES (NULL, '$thumbnail', '$destination', '$imgname', '$email', '$imgdate', '$username', '', '$phone','$codx','$cat')"
...
header("location:../wehr_new/Clinical_Page.php?docid=$username&patient_name=$imgname&patient_email=$email&pdate=$imgdate&phone=$phone&cat=$cat&cod=$codx");
}
...
?>
<div class="container">
 <p class="brand" style="margin-right:45px; margin-left:21px; font-weight:bold; color:#034482; font-size:20px; text-decoration:none;" href="index.php">My Health Records</p>
...
$webgrid=mysql_query("select * from doctor_detail where email='$username'");
$grid=mysql_fetch_object($webgrid);

    	 ?>
    	<br><a href="<?php echo $grid->myweb ?>">My Website</a>
        <a style="margin-left:20px;"href="<?php echo $grid->myclinic ?>">My Clinic</a>
    	
    	</div>
        	<div class="span4 text-right">
                <form method="post" action="search.php" class="" style="">
                        <input type="text" placeholder="Search an email ID of Patient
" name="srch_txt" class="input-medium search-query" style="height: 34px;width: 220px;">
            	...
            	<div class="row">
                   ...
                   <div class="span2" style="width:90px;"><h5>File</h5></div>
                    <span class="span2" style="width: 168px;"><h5 style="margin-left: 23px;">Upload Max 5 photos </h5> </span>
      				...
      				<div class="span2" style="width:110px; margin-left:28px;"><input required type="text"  name="name" class="span2" /></div>
                    <div class="span2"  style="width:110px;"><input type="email" name="email" class="span2" /></div>
                    <div class="span2"  style="width:110px;"><input type="text" maxlength="15" name="phone" class="span2" /></div>
                    <div class="span2"  style="width:110px;"><input type="text" id="cms1" onKeyUp="lookup_cms(this.value,this.id);" name="cat" value="Prescription" class="span2" /></div>
                  ...
                  <ul id="autoSuggestionsList_cus" style="color:rgb(60, 16, 199);margin-left: -124px;border:1px solid black;width: 227px;" onclick=getsrch()>
                  </ul>
                </div>
...
<div class="hero-unit h_top r_b" style="background:#54b4eb !important; margin-left:6px;">
<div class="row">
   ...
   <div class="span2" style="width:60px;"><strong>Date</strong></div>
    ...
    <div class="span1" style="width:57px;padding-left: 8px;"><strong>Send mail</strong></div>
    <div class="span1" style="width:57px;padding-left: 8px;"><strong>Delete</strong></div>
</div>
</div>
<?php
if(!empty($_REQUEST['f']))
...
<div class="row"  <?php if($alter%2!=0) { ?> style="margin-left:0px;background:#CEFCCB;" <?php }else { ?> style="margin-left:0px;background:#CCEAEA;" <?php }?>>
<?php
...
else
{
 ...
 echo '<a href="Emailer.php?email='.$details.'">Send</a>&nbsp;';
  ...
  echo '<a href="del1.php?page='.$docs_id.'&d='.$details.'"><img src="img/no.png" height="20" width="20" style="padding-left: 75px;margin-top: -20px;" title="Delete" ></a>';?>
...
<div class="row"  <?php if($alter%2!=0) { ?> style="margin-left:10px;width: 1000;background:#CEFCCB;" <?php }else { ?> style="margin-left:10px;background:#CCEAEA;width: 1000;" <?php }?>>
<?php
$docs_id=$file['docs_id'];
	$img=$file['img'];
	$cf=$file['current_folder'];
...
if ($ext=='pdf') {
	echo "<a href='$img'>Download PDF <a>";
}else
{
...
$x++;
      ...
      <div class="span2" style="width:57px;"><?php echo $cat;?></div>
        ...
        <a href="whatsapp://send" data-text="My Prescription:" data-href="<?php echo $link; ?>" class="wa_btn wa_btn_s" ><img src="img/whatsapp-icon.png" width="28px" height="28px"></a>
...
else
{
 ...
 echo '<a href="Emailer.php?email='.$details.'">Send</a>';
  ...
  <a class="btn btn-danger" style="padding-left: 12px;margin-top: -17px;height: 17px;width: 17px;margin-left: 70px;"; href="del1.php?page=<?php echo $docs_id; ?>&d=<?php echo $details; ?>">X</a>
